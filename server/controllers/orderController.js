...
exports.getOrdersHistory = (req, res) => {
    const sql = `
        ...
        JOIN products ON rentals.product_id = products.id
        JOIN users ON rentals.user_id = users.id
        WHERE rentals.status = 1;  -- You can keep or remove this condition based on your needs
    `;
    ...
    db.query(sql, (err, results) => {
        ...
        // No need to check for user-specific results, so this condition can be removed:
        // if (results.length === 0) {
        //     return res.status(404).json({ message: 'No orders found for this user' });
        // }
